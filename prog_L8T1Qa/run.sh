...
do 	test0=$(find $in_l8 -type f | grep $productL8$doy | grep tar.gz | wc -l)
	...
	then 
		...
		test1=$(find $in_l8 -type f | grep $productL8$doy | grep _B1.TIF | wc -l)
		test2=$(find $in_l8_qa -type f | grep $productL8$doy | grep _BQA.TIF | wc -l)
		# Get output file count
  		test3=$(find $out_l8 -type f | grep $out_l8$productL8$doy.tif | wc -l)
